...
export const Category = () => {
  ...
  const { data: catalogName, isLoading: catalogLoading } = useGetCatalog();
  const displayText = catalogLoading ? "загрузка..." : catalogName?.find((item) => item.name === slug)?.text;
  if (!data) return null;
  if (isLoading) return <Skeleton count={4} width={165} height={165} />;
  ...
  return (
    <div className='container'>
      <h1 className='pt-7 mb-7'>Главная / {displayText}</h1>
